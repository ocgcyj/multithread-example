            ...
            if msg.messageType() == "MarketDataEvents":
                ...
                for field in msg.asElement().elements():
                    ...
                    if field.name() in subscription_list:
                        ...
                        ticker_info_dict[ticker]["TIME_RT"] =  dt.datetime.strptime(timeStamp, '%Y/%m/%d %H:%M:%S') 
...
def execute():
    try:
        ...
        if position_flag == 0 and now.time() > dt.time(3, 30):
        ...
        if buy_count >= 0 and position_flag == 0 and net_diff < discount_threshold:
            ...
            buy_count += 1
            trade_log_df.loc[len(trade_log_df)] = [now, side, nav_last, exe_px, net_diff]
        ...
        if position_flag == 1 and now.time() > dt.time(3, 59):
            position_flag = 0
            side = 's'
            sell_count += 1
            trade_log_df.loc[len(trade_log_df)] = [now, side, nav_last, exe_px, net_diff]
...
trade_log_df = pd.DataFrame(columns = ['timestamp', 'side', 'nav', 'exe_px', 'net_diff'])
